...
mString = """KAEwTLSoYgcFQaJNhHVGxXDiQmzjfcpYbzxlWrVcqsmUbCunkfxZWDZjUZMiGqhRRiUvGmYmvnJIHEmbT
...
#print("".join(re.findall("[^A-Z]+[A-Z]{3}([a-z])[A-Z]{3}[^A-Z]+", mString)))
print("".join(re.findall("[^A-Z]+[A-Z]{3}([a-z])[A-Z]{3}[^A-Z]+", mString)))

"""
while index < len(mString)-7:

    answer = mString[index:index+7]
    if pattern.match(answer):
        urls.append("http://www.pythonchallenge.com/pc/def/"+answer+".html")
        answer = ""
    index += 1
